...
class TemporalModel(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        # """
        # Ensure lengths is long tensor on same device for masking later
        ...
        if self.use_attn:
            ...
            # take last valid timestep per example
            # lengths may contain values >=1
            idx = (lengths - 1).clamp(min=0)  # [B]
            # create index tensor [B,1,hidden_dim] to gather
            idx_expand = idx.unsqueeze(1).unsqueeze(2).expand(-1, 1, x_out.size(2))  # [B,1,H]
            last = x_out.gather(1, idx_expand).squeeze(1)  # [B, H]
